...
<!-- Naslov stranice i dugmad za prelazak na izte -->
  ...
  <div class="print_supplier">
    ...
    <a href="print_card_supplier.php?company=<?php echo $_GET['company']; ?>&date1=<?php echo $_GET['date1']; ?>&date2=<?php echo $_GET['date2']; ?>">
    <img src="pictures/printer.png">
    <p class="print_supplier_text">Štampanje kartice</p></a>
    <?php } ?>
    </div>
  <!-- Forma za pretragu podataka potrebnih za kartice kupaca -->
  <form  action="card_supplier.php" name="card_supplieradd" onsubmit="return validateCardSupplierAdd();">
  ...
  <!-- Pocetak tabele za prikaz podataka u kartici kupaca -->
   ...
   $sql=mysql_query("SELECT company_name,null,statement_number,statement_date,statement_value,payment from statement where company_name='$company'
    and payment='isplata' and statement_date BETWEEN '$date1' and '$date2' order by statement_date");
   ...
   {       
    ...
    <tr>
      ...
      <td></td>
      <td><span id="text" class="text">Izvod br. <?php echo $statement_number; ?></span></td>
    ...
    $sql=mysql_query("SELECT a.customer_id,a.customer_name,b.number,b.date_turnover,b.invoice_id,c.total_value from customer a 
    left join
    (
      select customer_id,invoice_id,number,date_turnover,invoice_type
      from invoice group by invoice_id 
    )b on a.customer_id=b.customer_id
    left join 
    (
      select invoice_id,customer_id,sum(total_value) total_value
      from invoice_item
      group by invoice_id
    )c on b.invoice_id=c.invoice_id where a.customer_name='$company' and b.invoice_type='ulazna' and date_turnover BETWEEN '$date1' and '$date2'  order by date_turnover
    ");
    ...
    {       
      ...
      <td></td>
      <td>
        <span><?php echo date('d-m-Y',strtotime($date_turnover)); ?></span>
      </td>
      <td>
  ...
  <tr>
    ...
    <td><?php $debit = 0;
      $query = "SELECT a.customer_id,a.customer_name,b.number,b.date_turnover,b.invoice_id,c.total_value from customer a 
    left join
    (
      select customer_id,invoice_id,number,date_turnover,invoice_type
      from invoice group by invoice_id 
    ...
    <td><?php $claimed = 0;
      $query = "SELECT * FROM statement where company_name='$company' and payment='isplata' and statement_date BETWEEN '$date1' and '$date2' ";
      $result = mysql_query($query);
      $rows = array();
      while($row = mysql_fetch_assoc($result))
        {$rows[] = array('statement_value' => htmlspecialchars($row['statement_value']),);
      $claimed += $row['statement_value'];}?>
      <p>Ukupno potražuje:  <?php echo $claimed; ?></p>
    </td>
    <!-- Automatski racuna saldo u kartici kupaca -->
    <td>Ukupan saldo: <?php echo $debit1 - $debit2; ?></td>
  </tr>
